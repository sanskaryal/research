...
"""
personalization.py
Federated Learning with client-side personalization.
- Averages the global model with the client's last updated model.
"""
...
CONFIG = {
    ...
    # Personalization
    "personalization_weight": 0.5,  # 0.0 = local only, 1.0 = global only
...
def get_filenames():
    ...
    csv_name = Path("results") / f"{base}_perso.csv"
    model_name = Path("trained_models") / f"{base}_perso.pth"
...
client_last_weights = {i: None for i in range(CONFIG["num_clients"])}
...
for r in range(1, rounds + 1):
    ...
    for cid in selected:
        ...
        if client_last_weights[cid] is not None:
            # Average global model with last client model
        ...
        client_last_weights[cid] = deepcopy(sd) # Store the new weights
